...
while true:
  ...
  PLAYERNAME=`grep 'Player connected' $INSTALL_DIR/7dtd.log | head -1 | awk '{print $4}' | cut -d= -f2`
  [[ ! -z $PLAYERNAME ]] && break;
...
[[ $7DTD_RENDER_MAP = true ]] && /7dtd-rendermap.sh $PLAYERNAME $STARTING_COORD $ENDING_COORD
